...
import { addToCartSchema, cartsSchema,cartItemSchema } from "../validators/validations"
...
export const getCartByUserId = async (req: Request, res: Response) => {
  try {
    ...
    if (cart.length === 0) {
      return res.status(200).json({ error: 'No cart entries found for the specified user ID' });
    }
...
export const addToCart = async (req: Request, res: Response) => {
  try{
    ...
    let cart = await CartsModel.findOne({ where: { user_id: userId, product_id:productId } });
...
export const clearCart = async (req: Request, res: Response) => {
  try {
    ...
    if(deleteCartItems[0] == 0){
      return res.status(400).json({error:"no cart items were found to delete"})
    }
...
export const decreaseQuantity = async (req: Request, res: Response) => {
  try{
    ...
    if (currentQuantity <= 1) {
      return res.status(400).json({ error: 'Product quantity cannot be decreased below 1' });
    }

    let updatingCart : any = await cart.update({ quantity: currentQuantity - 1 });
